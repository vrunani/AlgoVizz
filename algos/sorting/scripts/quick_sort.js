...
function quick_partition (start, end)
{
    ...
    div_sizes[i-1]=temp;
    const newNode = document.createTextNode(div_sizes[start]);
    const newNode1 = document.createTextNode(div_sizes[i-1]); 
    div_update(divs[start],div_sizes[start],"red");//Height update
    div_update(divs[i-1],div_sizes[i-1],"red");//Height update
    divs[start].replaceChild(newNode, divs[start].childNodes[0]); 
    divs[i-1].replaceChild(newNode1, divs[i-1].childNodes[0]);
